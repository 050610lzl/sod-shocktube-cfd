...
def steger_warming_flux(U, gamma=GAMMA):
    ...
    for i in range(n):
        ...
        # 正负分裂: lambda^+ = 0.5*(lambda + |lambda|_fix), lambda^- = 0.5*(lambda - |lambda|_fix)
...
def macormack_step(U, dx, dt, gamma=GAMMA):
    ...
    if forward_predictor:
        ...
        U_starstar = np.zeros_like(U)
        for i in range(1, n - 1):
            U_starstar[i, :] = U[i, :] - (dt / dx) * (F_star[i + 1, :] - F_star[i, :])
...
def rusanov_step(U, dx, dt, gamma=GAMMA):
    ...
    for i in range(1, n - 1):
        alpha = max(S[i], S[i + 1])
        U_new[i, :] = U[i, :] - (dt / dx) * (
            0.5 * (F[i + 1, :] + F[i, :]) - 0.5 * alpha * (U[i + 1, :] - U[i, :]) -
            (0.5 * (F[i, :] + F[i - 1, :]) - 0.5 * max(S[i - 1], S[i]) * (U[i, :] - U[i - 1, :]))
        )
...
def _riemann_flux_godunov(U_L, U_R, gamma=GAMMA):
    ...
    p_star = brentq(pressure_function, 1e-10, max(p_L, p_R) * 2.0 + 1.0, xtol=1e-12)
    ...
    if u_star > 0:
        ...
        F = np.array([rho_star * u_star,
                      rho_star * u_star ** 2 + p_star,
                      u_star * (rho_star * (p_star / ((gamma - 1.0) * rho_star) + 0.5 * u_star ** 2) + p_star)])
    else:
...
def _roe_average(U_L, U_R, gamma=GAMMA, entropy_fix=False):
    ...
    c_tilde = np.sqrt(np.maximum((gamma - 1.0) * (H_tilde - 0.5 * u_tilde ** 2), 1e-15))

    dU = U_R - U_L
    d_rho = dU[0]
    d_u = dU[1] - u_tilde * dU[0]
    d_p = (gamma - 1.0) * (dU[2] - u_tilde * dU[1] + 0.5 * u_tilde ** 2 * dU[0])
...
def roe_step(U, dx, dt, gamma=GAMMA, entropy_fix=False):
    ...
    for i in range(1, n - 1):
        D_right = _roe_average(U[i, :], U[i + 1, :], gamma, entropy_fix=entropy_fix)
        D_left = _roe_average(U[i - 1, :], U[i, :], gamma, entropy_fix=entropy_fix)
...
def _hllc_flux(U_L, U_R, gamma=GAMMA, entropy_fix=False):
    ...
    PVRS = 0.5 * (p_L + p_R) - 0.5 * (u_R - u_L) * 0.5 * (rho_L + rho_R) * 0.5 * (c_L + c_R)
    p_star = max(1e-10, PVRS)

    q_L = 1.0 if p_star <= p_L else np.sqrt(1.0 + (gamma + 1.0) / (2.0 * gamma) * (p_star / p_L - 1.0))
    q_R = 1.0 if p_star <= p_R else np.sqrt(1.0 + (gamma + 1.0) / (2.0 * gamma) * (p_star / p_R - 1.0))
    ...
    if entropy_fix:
        ...
        if p_star < p_L:
            ...
            if S_L_head < 0 and S_L > 0:
                ...
                S_L = (S_L ** 2 + eps ** 2) / (2.0 * eps) - (S_L_head ** 2 + eps ** 2) / (2.0 * eps)
    ...
    F_L = np.array([rho_L * u_L, rho_L * u_L ** 2 + p_L, u_L * (rho_L * (p_L / ((gamma - 1.0) * rho_L) + 0.5 * u_L ** 2) + p_L)])
    F_R = np.array([rho_R * u_R, rho_R * u_R ** 2 + p_R, u_R * (rho_R * (p_R / ((gamma - 1.0) * rho_R) + 0.5 * u_R ** 2) + p_R)])

    if S_L >= 0:
        return F_L
    elif S_star >= 0:
        D_star = rho_L * (S_L - u_L) / (S_L - S_star)
        U_star = D_star * np.array([1.0, S_star, p_L / ((gamma - 1.0) * rho_L) + 0.5 * (u_L ** 2 + (S_L - u_L) * (S_star - u_L))])
        return F_L + S_L * (U_star - U_L)
    elif S_R >= 0:
        D_star = rho_R * (S_R - u_R) / (S_R - S_star)
        U_star = D_star * np.array([1.0, S_star, p_R / ((gamma - 1.0) * rho_R) + 0.5 * (u_R ** 2 + (S_R - u_R) * (S_star - u_R))])
...
def hllc_step(U, dx, dt, gamma=GAMMA, entropy_fix=False):
    ...
    for i in range(1, n - 1):
        F_right = _hllc_flux(U[i, :], U[i + 1, :], gamma, entropy_fix=entropy_fix)
        F_left = _hllc_flux(U[i - 1, :], U[i, :], gamma, entropy_fix=entropy_fix)
        U_new[i, :] = U[i, :] - (dt / dx) * (F_right - F_left)
...
def tvd_minmod_step(U, dx, dt, gamma=GAMMA):
    ...
    for i in range(n - 1):
        ...
        # 右界面值
        if i + 1 < n - 1:
            U_right_half[i, :] = U[i + 1, :] - 0.5 * phi[i + 1, :] * delta[i + 1, :]
        else:
    ...
    for i in range(1, n - 1):
        ...
        F_right = _tvd_riemann_flux(U_left_half[i, :], U_right_half[i, :], gamma)
        F_left = _tvd_riemann_flux(U_left_half[i - 1, :], U_right_half[i - 1, :], gamma)
...
def solve_with_scheme(scheme_name, U, x, dx, t_final=0.2, cfl=0.8, gamma=GAMMA):
    """

...
**文献依据**: Laney (1998) [3], OneFlow-CFD [4]

### 4.1 功能描述

在每步时间推进后, 对计算域边界节点施加零梯度外推边界条件。
...
实现位置: [boundary_handler.py#L11-L29](file:///e:/trae_project/a/src/boundary_handler.py#L11-L29)

### 4.4 设计决策

- **原地修改**: 直接修改传入的 `U` 数组, 同时返回该数组, 减少内存复制
- **所有分量统一处理**: 密度、动量、能量三个守恒量均采用零梯度外推
- **物理合理性**: Sod 激波管在 `t=0.2` 时, 激波和稀疏波尚未到达边界, 零梯度边界是合理的近似
- **无需周期性条件**: Sod 问题的标准边界条件即为外推边界 (见 OneFlow-CFD [4])
